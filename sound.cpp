...
/*------------------------------------------------------------------------------
   グローバル変数の定義
------------------------------------------------------------------------------*/
static IXAudio2 *g_pXAudio2 = nullptr;									// XAudio2オブジェクトへのインターフェイス
static IXAudio2MasteringVoice *g_pMasteringVoice = nullptr;			// マスターボイス
...
int LoadSound(char* pFilename)
{
	...
	{
		MessageBox(nullptr, "最大読み込み数を超過", "警告！", MB_ICONWARNING);
		return -1;
	}
	...
	// サウンドデータファイルの生成
	hFile = CreateFile(pFilename, GENERIC_READ, FILE_SHARE_READ, nullptr, OPEN_EXISTING, 0, nullptr);
